...
import carla
from gan_controller import VehiclePIDController
from agents.tools.misc import distance_vehicle, draw_waypoints
import logging
logging.basicConfig(filename='local_planner_behavior.log',level=logging.DEBUG)
...
class LocalPlanner(object):
    ...
    def _init_controller(self):
        ...
        logging.info('local planner initialized with speed limit: %.2f' % self._target_speed)
    ...
    def set_global_plan(self, current_plan, clean=False):
        ...
        logging.info('local planner behavior: in set_global_plan')
        if clean:
    ...
    def get_incoming_waypoint_and_direction(self, steps=3):
        ...
        """
        logging.info('local_planner in get_incoming_waypoint')
        #load up local wpt
        #self._wps = someFunction
        #self.waypoints_queue.append(self._wps)
        if len(self.waypoints_queue) > steps:
            #try GAN waypoints here
    ...
    def run_step(self, target_speed=None, debug=True):
        ...
        #self.target_waypoint = self.target_waypoint+self._current_waypoint
        logging.info('target waypoint: %s', str(self.target_waypoint.transform.location))
        if target_speed > 50:
