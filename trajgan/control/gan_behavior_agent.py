...
import numpy as np
import carla
from gan_agent import Agent
from gan_local_planner_behavior import LocalPlanner, RoadOption
from gan_global_route_planner import GlobalRoutePlanner
from gan_global_route_planner_dao import GlobalRoutePlannerDAO
from agents.navigation.types_behavior import Cautious, Aggressive, Normal

from agents.tools.misc import get_speed, positive
import logging
logging.basicConfig(filename='behavior_agent.log',level=logging.DEBUG)
...
class BehaviorAgent(Agent):
    ...
    def __init__(self, vehicle, ignore_traffic_light=True, behavior='normal'):
        ...
        self.min_speed = 5
        self.behavior = None
        self._sampling_resolution = 7 #4.5
        ...
        if behavior == 'cautious':
            ...
            logging.info('behavior agent set to normal')
    ...
    def set_destination(self, start_location, end_location, clean=False):
        ...
        """
        logging.info('behavior agent: setting destination')
        if clean:
        ...
        logging.info('behavior agent calling local planner behavior')
        self._local_planner.set_global_plan(route_trace, clean)
        logging.info('behavior agent: destination set')
    ...
    def _trace_route(self, start_waypoint, end_waypoint):
        ...
        if self._grp is None:
            ...
            #GAN generated waypoints here
            dao = GlobalRoutePlannerDAO(
                wld.get_map(), sampling_resolution=self._sampling_resolution)
            logging.info(dao.get_topology())
            grp = GlobalRoutePlanner(dao)
            grp.setup()
            self._grp = grp
            logging.info('behavior agent: trace_route grp set')

        # Obtain route plan
        route = self._grp.trace_route(
            start_waypoint.transform.location,
            end_waypoint.transform.location)
        logging.info('behavior agent: Global route found!')
        return route
    ...
    def run_step(self, debug=False):
        ...
        Execute one step of navigation.
            ...
            :return control: carla.VehicleControl
        """
        logging.info('behavior agent in run_step')
        control = None
